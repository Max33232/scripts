-- FE TELEKINESIS TOOL - СТРОГО ПО ТВОЕМУ ШАБЛОНУ (АВТОР: TEST)
...
table.insert(cors,sandbox(LocalScript3,function()
	...
	BP.maxForce=_VTR_new(9e9,9e9,9e9)
	BP.P=BP.P*3
	dist=nil
	point=_Ins("Part")
	point.Locked=true
	...
	local onButton1Down=function(mouse)
		...
		while (mousedown==true) do
			if (mouse.Target~=nil) then
				...
				if (t.Anchored==false and not t:IsDescendantOf(game:GetService("Players").LocalPlayer.Character)) then
					object=t
