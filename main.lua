-- TELEKINESIS С FLEUNT МЕНЮ И ПАРОЛЕМ (АВТОР: TEST)
...
-- ЗАПРОС ПАРОЛЯ
userInput = game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "Введите пароль",
    Text = "Пароль: TEST",
    Duration = 5
})

userInput = game:GetService("UserInputService").InputBegan:Wait()
local inputBox = Instance.new("TextBox")
inputBox.Parent = game:GetService("CoreGui")
inputBox.Size = UDim2.new(0, 200, 0, 30)
inputBox.Position = UDim2.new(0.5, -100, 0.5, -15)
inputBox.PlaceholderText = "Введите пароль"
inputBox.Text = ""
...
inputBox.FocusLost:Connect(checkPassword)

-- ОСНОВНАЯ ФУНКЦИЯ ТЕЛЕКИНЕЗА
function loadTelekinesis()
    ...
    Fluent:Notify({
        Title = "Telekinesis",
        Content = "Инструмент в инвентаре! Нажми ПКМ на блок",
        Duration = 4
    })
end
